...
writer=manimation.FFMpegWriter(fps=FPSN) 
#frames1=[]
...
with writer.saving(fig, "schematic_output.mp4", dpi=200):
	...
	for k in range(kSTART, kEND):
		...
		def hex_heatmap(n_y=50, hex_size=1):
			...
			#fig, ax = plt.subplots(figsize=(300*0.6,100*0.6))
			...
			plt.title(f"Time = {time[k]} ka", fontsize=14)
			ax.set_aspect('equal')
			ax.set_xlim(-hex_size, (n_x - 1) * dx + hex_size)   # Show columns roughly between 10 and 30
			ax.set_ylim(-5*hex_size, (n_y - 1) * 2*dy + 2*hex_size) 
			ax.axis('off')
...
#ani.save("A_video_output_schematic_NAME.mp4", writer=FFwrite) 
