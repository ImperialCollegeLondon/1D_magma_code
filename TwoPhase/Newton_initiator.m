...
% old enthalpy
syms OLD_ent
trans_enthalpy=((cp*T_1+Lf*phi_1)-OLD_ent-Lf*((phi_0+phi_1)/2*ufi-(phi_1+phi_2)/2*ufi2)/dzi_1*dt-kt*2/dzi_1*((T_2-T_1)/(dzi_2+dzi_1)-(T_1-T_0)/(dzi_1+dzi_0))*dt)/Lf;
...
if Has_volatile==1
    ...
    syms OLD_com2
    syms kf
    Variables=[umi_1; umi_2; ufi; ufi2; phi_0; phi_1; phi_2; cs2_0; cs2_1; cs2_2; cl2_0; cl2_1; cl2_2; S_0; S_1; S_2];
    trans_comp2=(phi_1*cl2_1+(1-phi_1)*cs2_1+S_1)-OLD_com2-(ufi*(phi_0*cl2_0+phi_1*cl2_1)/2-ufi2*(phi_1*cl2_1+phi_2*cl2_2)/2+umi_1*((1-phi_0)/2*cs2_0+(1-phi_1)/2*cs2_1)-umi_2*((1-phi_1)/2*cs2_1+(1-phi_2)/2*cs2_2))/dzi_1*dt...
        -kf*2/dzi_1*((S_2-S_1)/(dzi_2+dzi_1)-(S_1-S_0)/(dzi_1+dzi_0))*dt;
...
if Has_volatile==1
    Sat=a0*T_1+b0-cl2_1;
    condition=Sat+S_1-sqrt(Sat^2+S_1^2); %condition=0 means either Sat>0 and S=0;  or Sat=0 and S>0
    
    Variables=[T_1; cl2_1; S_1];
...
if Has_volatile==1    
    ...
    Par=cl2_1*D1-cs2_1;
    condition=Sat+Par-sqrt(Sat^2+Par^2); %condition=0 means either Sat>0 and Par=0;  or Sat>0 and Par=0
    
    Variables=[cs_1; cs2_1; cl2_1];
