% HH 2/2/2022
% Solving the transport equation in a control volume manner
function results=CV_transport_scaled_v2_master(dz,value_old,phi,phi_old,u,k,dt,Source,BC_type,BC_value)
    % this function solves transport equation with saturation
    ...
    for i=0:N-3             
      ...
      indexi(i*3+3)=i+2; indexj(i*3+3)=i+3; value(i*3+3)=      -coe*2*kk(i+2)*phi(i+3)/(dz(i+2)+dz(i+3))/dz(i+2);
    ...
    switch BC_type(1) %Lower boundary
        ...
        case 3
%             if u(2)>0
%                 indexi=[indexi, 1,1]; indexj=[indexj,1,2]; value=[value, phi(1)/dt+u(2)*phi(1)/dz(1)+coe*2*kk(1)*phi(1)/(dz(1)+dz(2))/dz(1)+coe*k(1)*phi(1)/dz(1)^2,  -coe*2*kk(1)*phi(2)/(dz(1)+dz(2))/dz(1)];   %Lower no flux
%             else
%                 indexi=[indexi, 1,1]; indexj=[indexj,1,2]; value=[value,            phi(1)/dt+coe*2*kk(1)*phi(1)/(dz(1)+dz(2))/dz(1)+coe*k(1)*phi(1)/dz(1)^2,u(2)*phi(2)/dz(1)-coe*2*kk(1)*phi(2)/(dz(1)+dz(2))/dz(1)];   %Lower no flux  
%             end  
            if u(2)>0
...
%             indexi=[indexi, 1,1]; indexj=[indexj,1,2]; value=[value, phi(1)/dt+phi(1)*u(1)+k(1)*coe*phi(1)/(dz(1)+dz(2)),-k(1)*coe*phi(2)/(dz(1)+dz(2))];
    end
    ...
    switch BC_type(2) %Upper boundary
        ...
        case 3
%             if u(N)>0
%                 indexi=[indexi, N,N]; indexj=[indexj,N-1,N]; value=[value, -u(N)*phi(N-1)/dz(N)-coe*2*kk(N-1)*phi(N-1)/(dz(N)+dz(N-1))/dz(N),           phi(N)/dt+coe*2*kk(N-1)*phi(N)/(dz(N)+dz(N-1))/dz(N)+coe*k(N)*phi(N)/dz(N)^2];   %Upper no flux
%             else
%                 indexi=[indexi, N,N]; indexj=[indexj,N-1,N]; value=[value,            -coe*2*kk(N-1)*phi(N-1)/(dz(N)+dz(N-1))/dz(N),phi(N)/dt-u(N)*phi(N)/dz(N)+coe*2*kk(N-1)*phi(N)/(dz(N)+dz(N-1))/dz(N)+coe*k(N)*phi(N)/dz(N)^2];   %Upper no flux
%             end            
            if u(N)>0
...
%             indexi=[indexi, N,N]; indexj=[indexj,N,N-1]; value=[value, phi(N)/dt+phi(N)*u(N+1)+k(N)*coe*phi(N)/(dz(N)+dz(N-1)),-k(N)*coe*phi(N)/(dz(N-1)+dz(N))];
    end
...
%             rhs(1)=rhs(1)-(1-coe)*k(1)*phi(1)*value_old(1)/dz(1)^2;
    ...
    switch BC_type(2) %Lower boundary
        case 1
