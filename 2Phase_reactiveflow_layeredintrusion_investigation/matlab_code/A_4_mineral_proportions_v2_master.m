function [ol, opx, cpx, feld,ol_mg,px_mg,opx_mg,cpx_mg,feld_mg,ol_mn,sol_mn] = A_4_mineral_proportions_v2_master(ol,opx,cpx, feld, ...
...
elseif cb>crit_cb2
    if T >= crit_T2
        ...
        ol_mn = 0;
        sol_mn = 0;
    elseif T<copx_T_m_1h*cb + copx_T_c_1h
        if copx_T_m_1h*cb + copx_T_c_1h>=crit_T2
           ol_mg = s1m*(copx_T_m_1h*cb+copx_T_c_1h)+s1c;
            ol_mn = ol_mn_m*T + ol_mn_c;
            sol_mn = sol_mn_b + sol_mn_a / (sol_mn_k - T);
        else
            ol_mg = s12m * (copx_T_m_1h*cb + copx_T_c_1h) + s12c;
            ol_mn = ol_mn_m*T + ol_mn_c;
            sol_mn = sol_mn_b + sol_mn_a/(sol_mn_k - T);
        end
    else
