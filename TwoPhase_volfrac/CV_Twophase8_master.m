...
if Restart==1
   ...
   % Plot_settings;
    ...
    %% CAB - Sill intrusion
    SillRate = injection_rate*1e6*Year;
    ...
    %           H=H_old;
      ...
      %% Update melt fraction and composition due to phase_diagram
            ...
            if FourMPD==1
                ...
                if N<50 || Use_parallel==0
                    for i=1:N
                        [phi_wt(i),T(i),C_all(i), C_all(i+N), TYPE(i), Tl(i), Ts(i)]=poro_component_solve_4_components_v2_master(Cb(i), H(i), Lf, cp, min_mgo, max_mgo, crit_T1, crit_T2,...
                   ...
                   parfor i=1:N
                       warning('off', 'all');
                       %set(0,'DefaultFigureVisible','off')
                       [phi_wt(i),T(i),C_all(i), C_all_dummy(i), TYPE(i), Tl(i), Ts(i)]=poro_component_solve_4_components_v2_master(Cb(i), H(i), Lf, cp, min_mgo, max_mgo, crit_T1, crit_T2,...
                                                                 crit_cb2, crit_cb3, crit_cb4,crit_cb4_OG, crit_cb5,liq_k1, liq_a1, liq_b1,liq_k2, liq_a2, liq_b2, liq_k3, liq_a3, liq_b3,...
                                                                lm1, lc1, lm3, lc3, lm8, lc8, mk, ck, BT, BC, DT, DC,JC, olpx_C,olpxm, olpxc, s5T, sol_k2,liq_P2_C,s1m,s1c, ...
                                                                lowest_end_T, liq_P3_C,l13_k,l13_a,l13_b,lowest_k,s8m,s8c,crit_T3,liq_P4_C, Precision_PD,step_size);
                    end
                ...
                vv=['Output ',num2str(Time/Year/1000), ' ka'];
                    ...
                    elseif FourMPD==1
                        ...
                        if To_cal_mc==1
                            ...
                            fprintf(File_Out, '%10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s  \t %10s \t %10s\t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s \t %10s  \t %10s \t %10s\t %10s \t %10s  \t %10s \t %10s\t %10s \n', 'Depth (km)', ...
                                'Melt frac.', 'Melt frac. wt', 'Enth.', 'Temp. (C)', 'Ts (C)', 'Tl (C)', 'Cb', ...
                            ...
                            for i=1:1:N
                                fprintf(File_Out, '%10.3f \t %10.4f \t %10.4f \t %3e \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f  \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \t %10.4f \n', ...
                                    [cellz(i)/1000-Base_crust; phi(i); phi_wt(i); H(i); ...
