...
Inputs= readtable('1AA_2phase_master_input_validation.txt');
...
if HHJPet==1
    Inputs= readtable('1AA_2phase_HHJPet_validation.txt');
    [r,~] = size(Inputs);
    names=string(Inputs.Var2);
    Number=Inputs.Var3;
    for i=1:r
...
elseif SSPD==1
    Inputs= readtable('1AA_2phase_SSPD_validation.txt');
    is_eutectic=0;
    [r,~] = size(Inputs);
    names=string(Inputs.Var2);
    Number=Inputs.Var3;
...
% injection T and H needs to be determined after volatile content 
...
T(:)=injection_T;
Cb(:)=5e-2;
injection_H=injection_T*cp+injection_phi*Lf;
%%
H=Lf*phi+cp*T;
Cl=zeros(N,1);
Cs=zeros(N,1);
...
for i=1:N
    ...
    if HHJPet==1
        [~,~,Cl(i), Cs(i),~, Tl_local(i),Ts_local(i)]=poro_component_solve_JPET_master(Cb(i), H(i),A1, B1, C1, A2, B2, C2, ae, Lf, cp,Precision_PD);
    end

    if SSPD==1
        [~,~,Cl(i), Cs(i),~, Tl_local(i),Ts_local(i)] = poro_component_solve_solid_master(Cb(i), H(i), A1, Ts0(i)-A1-Tl0(i), Tl0(i), alpha, n_PD ,Lf, cp, Precision_PD,step_size);
    end
...
if Use_Newton==1
    ...
    load('Jacobians_for_Newton_for_validation.mat')
