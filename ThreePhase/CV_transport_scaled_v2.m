function results=CV_transport_scaled_v2(dz,value_old,phi,phi_old,u,k,dt,Source,BC_type,BC_value)
...
%     dz=dz';
    ...
    for i=0:N-3             
      ...
      indexi(i*3+2)=i+2; indexj(i*3+2)=i+2; value(i*3+2)=phi(i+2)/dt +coe*2*kk(i+1)*phi(i+2)/(dz(i+1)+dz(i+2))/dz(i+2)+coe*2*kk(i+2)*phi(i+2)/(dz(i+2)+dz(i+3))/dz(i+2);
